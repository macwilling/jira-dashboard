...
import { ACCENT, Panel, StageChip } from "./ui";
...
export default function SprintBoardScene() {
  const { tickets: allTickets, teamMembers, sprint } = useTicketData();
  ...
  // ---- sound (the toggle + audio unlock live in the kiosk chrome) ----
  const { soundOn } = useKiosk();
        ...
        <Panel
          ...
          titleRight={
            sprint && sprintMeta ? (
              <SprintRisk
                day={sprintMeta.day}
                total={sprintMeta.total}
                doneFrac={tickets.length ? doneCount / tickets.length : 0}
              />
            ) : undefined
          }
          className="flex-[2.4]"
...
/**
 * Sprint risk verdict for the board header: compares fraction of work done
 * against fraction of the sprint elapsed. Green when on/ahead of pace, amber
 * when slipping, red when clearly behind — so "we're behind" is a glance.
 */
function SprintRisk({
  day,
  total,
  ...
  const timeFrac = total > 0 ? day / total : 0;
  const gap = doneFrac - timeFrac;
  const tone = gap >= -0.1 ? "good" : gap >= -0.25 ? "warn" : "bad";
  const color =
    tone === "good" ? "#3fb950" : tone === "warn" ? "#d29922" : "#f85149";
  const verdict =
    tone === "good"
      ? "On track"
      : tone === "warn"
  ...
  return (
    <span
      className="ml-auto flex items-center gap-[0.4em] rounded-full border px-[0.6em] py-[0.15em] text-[0.55em] font-semibold normal-case tracking-normal"
      style={{ color, borderColor: `${color}55`, background: `${color}14` }}
      title={`${Math.round(doneFrac * 100)}% of work done at ${Math.round(
        timeFrac * 100
      )}% of the sprint`}
    >
      <span
      ...
      <span className="font-normal text-muted-foreground">
        {Math.round(doneFrac * 100)}% done · {Math.round(timeFrac * 100)}% elapsed
      </span>
    </span>
  );
}
