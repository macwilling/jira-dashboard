...
- `/wallboard` — full-screen TV dashboard (no AppShell chrome) rotating through three screens (sprint board / developer activity / My Day calendar agenda; keys 1/2/3 pin, 0 resumes): story-grouped momentum sprint board (subtasks attach to stories via `Ticket.parentKey`; Jira statuses map to display stages in `app/wallboard/stages.ts` — Resolved="Code Review", unknown="Done"), change feed with toasts/ding, GitHub PR KPIs (`/api/github/prs/summary`), Datadog RUM insights (`/api/datadog/insights`, `lib/datadog/client.ts`), Google Calendar day agenda with a flight-path strip, day stats, Google Tasks due-today rail, and a next-workday pre-read (`/api/google/calendar/today`, `/api/google/tasks/today`; the header meeting-countdown pill turns green once all of today's meetings have ended). Polls tickets every 60s (shares the provider's SWR cache key), stats every 5 min; change detection is a client-side snapshot diff (`app/wallboard/feed.ts`)
